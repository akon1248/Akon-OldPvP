...
Combat:
  ...
  AttackSounds: #プレイヤーがエンティティを殴ったときに発生するサウンドの設定
    crit: false #クリティカルヒットのサウンドを発生させるかどうか
    knockback: false #ノックバックヒットのサウンドを発生させるかどうか
    nodamage: false #無敵状態のエンティティを殴ったときのサウンドを発生させるかどうか
    strong: false #強攻撃のサウンドを発生させるかどうか
    weak: false #弱攻撃のサウンドを発生させるかどうか
Projectile:
...
Potion:
  old-potion-duration: true #ポーションの効果時間を1.8以前と同じにするかどうか
...
Enchantment:
  ...
  old-enchanting-table: true #エンチャント内容が表示されるたびに付与されるエンチャントがリセットされるかどうか
  old-sharpness: true #ダメージ増加エンチャントの効果を1.8以前と同じにするかどうか
Item:
  old-tool-damage: true #ツールのダメージを1.8以前と同じにするかどうか
  old-item-info: true #防具の防御力を非表示にし、アイテムの攻撃力の表記を1.8以前と同じにするかどうか
  old-diamond-armors: true #ダイヤモンド防具の防具強度を無効化します
Utilities:
